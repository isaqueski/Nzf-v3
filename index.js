...
const arrayBulan = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oltubro`, 'Novembro", 'Desembro]
const bulan = arrayBulan[moment().format('MM') - 1]
const config = {
    XBOT: '❉bruxinho❉', 
    instagram: 'https://instagram.com/affis_saputro123', 
    nomer: 'wa.me/+51924318593',
    youtube
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'lionlogo':
                      ...
                      break
                case 'truth':
					const trut =['Você já gostou de alguém? Quão mais?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
                ...
                case 'anjing':
					...
					break
                case 'indohot':
                if (!isNsfw) return reply('isNsfw não está ativo')
            ...
            client.sendFile(from, sesPic, 'sessão PING', '^_^...', id)
            break	
				case 'gtts':	
				case 'tts':
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('Bem, falha; (tente repetir ^_^')
			...
			case 'loli': 
				    try {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lembrar! Citai Lolimu'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Alterado o icone do grupo com sucesso')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
       ...
       client.sendMessage(from, 'Este é o número do meu proprietário> _ <, não envie spam ou bloqueie-me',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Sucesso, mude a descrição do Grupo`, text, {quoted: mek})
                break
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *TEM CERTEZA* ?') 
					if (args.length < 1) return reply(' *QUERO A TAG DO CLONE!!!* ')
